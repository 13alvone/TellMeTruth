...
3. Run:
       ...
       gmailtail --credentials credentials.json --token token.json --setup

   Follow the browser prompt to authorize access to your Gmail account. The command produces `token.json` referenced by `GMAILTAIL_TOKEN`.
...
- To change polling interval (in seconds), set `INTERVAL` in `.env`.
        ...
        Example .env:
                ...
                GMAILTAIL_CREDENTIALS=/app/credentials.json
                # Token generated by `gmailtail --setup`
                GMAILTAIL_TOKEN=/app/token.json
                GMAIL_DESIRED_SENDERS=trusted1@example.com,trusted2@icloud.com
                YTDLP_COOKIES_FILE=/app/cookies.txt
                INTERVAL=3600
